...
> ⚠️ **Reference(REF2VA) 모드는 현재 ComfyUI 빌드(5727)에서 동작하지 않습니다.**
> 해상도·체크포인트(FL2VA/Ref2VA)·가속·패치를 모두 바꿔가며 확인했으나 동일하게 실패합니다:
> 오디오 레퍼런스가 없으면 `VAEDecodeAudio` 텐서 불일치, 있으면 `SamplerCustomAdvanced` shape 불일치.
> **이 노드가 만드는 그래프 문제가 아니라 ComfyUI 코어의 MiniMax 레퍼런스 경로 문제**이며(참조 워크플로우와
> 배선 동일, 스키마 검증 통과), 모드는 남겨두되 UI에 경고를 표시합니다.
> **Text only · First/Last Frame · 클립 릴레이 · 합본은 정상 동작합니다.**
