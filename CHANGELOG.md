...
### 의존성 경고에 복사 버튼 + 실행 안내 팝업

- 노드의 빨간/주황 경고 배너에서 경로 두 줄(Windows / Mac·Linux) 각각 옆에 **⧉ Copy**
  버튼. 누르면 그 명령이 클립보드에 복사되고, **단계별 팝업**이 뜹니다 —
  "Win+R → cmd → 붙여넣기 → Enter → ComfyUI 재시작". 팝업에는 `Copy again` / `Got it`.
- 팝업은 자동으로 사라지지 않습니다(터미널로 전환한 상태에서 읽어야 하므로).
