...
| [ComfyUI-MiniMaxH3-FirstBlockCache](https://github.com/duckyshell/ComfyUI-MiniMaxH3-FirstBlockCache) 🧪 | 스텝 재사용 캐시<br><sub>step-reuse cache</sub>| MiniMax H3 (선택)<br><sub>optional</sub>|
...
> ⚠️ **이미지 노드의 RUN 큐와는 다른 구조입니다 — 브라우저 탭이 계속 열려 있어야 합니다.**
> 이미지 생성 노드는 ComfyUI 상단 RUN으로 그래프 전체를 서버에 한 번에 제출하므로, 브라우저를 닫아도 서버가 큐에 쌓인 걸 계속 처리합니다(이미지는 느려도 보통 2분 내외로 끝나서 체감이 잘 안 될 뿐입니다). 반면 MiniMax H3의 클립 릴레이(그리고 ⏭ Next Gen 대기 큐)는 **다음 클립의 그래프를 "이전 클립의 실제 결과"를 보고 그때그때 만들기 때문에** 미리 통째로 제출해둘 수가 없고, 그 판단을 브라우저 쪽 JS가 매 클립마다 내립니다. 그래서 지금 실행 중인 클립 하나는 브라우저를 닫아도 서버에서 끝까지 돌지만, **그다음 클립을 이어서 큐에 넣어줄 코드가 사라져 거기서 멈춥니다.** 영상은 클립이 여러 개면 총 소요 시간이 수 분~수 시간까지 갈 수 있으니, 탭을 (백그라운드/최소화는 괜찮지만) 완전히 닫지 마세요. **컴퓨터/모니터가 절전모드(잠자기)로 들어가는 것도 탭을 닫는 것과 같은 문제**입니다 — 그 시점의 클립은 서버에서 끝까지 돌지만 다음 클립을 이어서 넣어줄 브라우저 쪽 실행이 멈추므로, 긴 런을 돌릴 때는 절전모드를 꺼두세요. (완화책으로 실행 중에는 거의 무음에 가까운 낮은 볼륨의 오디오를 백그라운드에서 계속 재생해, 브라우저/OS가 탭을 완전히 절전 상태로 넘기지 않도록 유도합니다 — 100% 보장은 아니니 긴 런은 절전모드 자체를 꺼두는 쪽을 권장합니다.) 페이지를 그냥 **새로고침**한 경우에는 진행 상황이 저장되어 있어 자동으로 이어집니다.
> **This is a different structure from the RUN queue on image nodes — the browser tab has to stay open.** Image nodes submit the whole graph to the server in one shot via RUN, so the server keeps processing the queue even with the browser closed (this is easy to miss since an image usually finishes within ~2 minutes regardless). MiniMax H3's clip relay (and the ⏭ Next Gen queue) is different: the next clip's graph depends on the *actual result* of the previous one, so it can't be submitted upfront — that decision is made by the browser's own JS, clip by clip. The clip currently in flight will finish server-side even if you close the tab, but nothing is left to queue the next one, so the run stops there. A video run with several clips can take anywhere from minutes to hours, so keep the tab open (backgrounded/minimized is fine, just not closed). **The computer/monitor going to sleep causes the exact same problem as closing the tab** — the clip in flight still finishes server-side, but nothing is left running in the browser to queue the next one, so disable sleep for a long run. (As a mitigation, a near-silent low-volume tone plays in the background for the whole run, nudging the browser/OS away from fully suspending the tab — not a guarantee, so disabling sleep is still the reliable option for a long run.) A plain page **refresh** is safe — saved progress picks the run back up automatically.
...
| **가속 / 업스케일**<br><sub>Accel / Upscale</sub>| Turbo LoRA(FL2VA 전용) · SolAttn · Spectrum · None / Upscale Model · RTX VSR · **FlashVSR VSR** 🆕 · None. **Turbo를 켜면 H3 Cache는 자동으로 꺼집니다**(4스텝 스케줄은 캐시 재사용 임계값에 도달하지 않아 같이 켜면 속도/품질 비교가 왜곡됨). FlashVSR는 타일 기반이라 실시간 프리뷰가 없고(진행 중 배너로 대체), 좌측 패널과 갤러리 양쪽에서 모델/모드/배율/컬러픽스/타일 사이즈/타일 오버랩/시드/시드 컨트롤 8개 값을 설정합니다 — 기본값은 16GB 카드 기준 2배·384px 타일(약 7분 30초/클립), 타일을 256px로 줄이면 3배도 가능합니다(약 18분/클립)<br><sub>selectable acceleration and upscaling. **Turbo forces H3 Cache off** — its 4-step schedule never reaches the threshold H3 Cache reuses steps at, so leaving both on skews any speed/quality comparison. FlashVSR is tile-based with no live preview of its own (a waiting banner shows instead); both the left panel and the gallery expose the same 8 fields — model/mode/scale/color fix/tile size/tile overlap/seed/seed control. Shipped default is 2x/384px tiles on a 16GB card (~7m30s/clip); dropping the tile to 256px enables 3x (~18min/clip)</sub>|
...
| [ComfyUI-MiniMaxH3-FirstBlockCache](https://github.com/duckyshell/ComfyUI-MiniMaxH3-FirstBlockCache) | 스텝 캐시 가속<br><sub>step-reuse cache</sub>| 해당 옵션만 비활성<br><sub>that option disabled</sub>|
...
| **ComfyUI-MiniMaxH3-FirstBlockCache** 🧪 | 스텝 재사용 캐시<br><sub>step-reuse cache</sub>| MiniMax H3 | [duckyshell/ComfyUI-MiniMaxH3-FirstBlockCache](https://github.com/duckyshell/ComfyUI-MiniMaxH3-FirstBlockCache) |
| **ComfyUI-PlagueKind-Nodes** 🧪 | H3 SLA Attention (블록-스파스)<br><sub>H3 SLA Attention (block-sparse)</sub>| MiniMax H3 | [PlagueKind/ComfyUI-PlagueKind-Nodes](https://github.com/PlagueKind/ComfyUI-PlagueKind-Nodes) |
...
| **ComfyUI-FlashVSR_Ultra_Fast** 🧪 | 타일 기반 디퓨전 VSR (`FlashVSRInitPipe`/`FlashVSRNodeAdv`)<br><sub>tiled diffusion video super-resolution</sub>| MiniMax H3 · Upscale=FlashVSR VSR | [lihaoyun6/ComfyUI-FlashVSR_Ultra_Fast](https://github.com/lihaoyun6/ComfyUI-FlashVSR_Ultra_Fast) |
...
- [duckyshell/ComfyUI-MiniMaxH3-FirstBlockCache](https://github.com/duckyshell/ComfyUI-MiniMaxH3-FirstBlockCache) — 스텝 재사용 캐시 / step-reuse cache
