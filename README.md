...
TJ NODE STUDIO ONE이 배선 없는 올인원 생성 UI라면, **TJ_NODE는 대규모 워크플로우 전체를 구조화하는 Architecture Toolkit**입니다.

### 주요 노드

| 노드                          | 설명                                                         |
| ----------------------------- | ------------------------------------------------------------ |
| **Prompt Studio (TJ)**        | LLM 기반 프롬프트 생성 · 강화 · 이미지→프롬프트 변환. GGUF / llama.cpp / ComfyUI TextGenerate 백엔드 지원 |
| **Scene Maker (TJ)**          | Visual Beat 기반 프롬프트 구조 노드. KO/EN/JP/CN 자동 번역, Embedded Get/Set |
| **Prompt Show & Locker (TJ)** | 프롬프트 시각화 · 복사 · 잠금 노드                           |
| **Wireless Architecture**     | Fake-Wire 무선 라우팅 · Multi Router · Batch Workflow · Save Pipeline · HTML5 Overlay UI |

두 패키지를 함께 사용하면 LLM 프롬프트 생성부터 이미지 생성 · 편집 · 업스케일 · 저장까지 전체 파이프라인을 깔끔하게 구성할 수 있습니다.
