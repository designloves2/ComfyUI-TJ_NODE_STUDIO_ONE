...
**두 번째 크래시 (실사용자 발생, 2026-09-12 오전)**: 렌더와 전혀 안 겹쳐도 크래시가 또 났다 —
사용자가 새로 만든 Pick Faces UI에서 **큰 클립**을 갤러리로 골라 스캔했을 때. 로그
(`comfyui_8188.prev.log`)를 확인해보니 GPU는 3%만 쓰는데 CPU/RAM(92%)만 계속 오르다가 응답
자체가 끊김 — `/h3_facerefine/scan`(팩 자체 코드)이 얼굴 검출 전에 **영상 전체를 프레임마다
디코드해서 메모리에 통째로 올려놓기 때문**. 이건 렌더 동시 실행과 무관한, 스캔 자체의 별개
문제 — `ui_gallery_minimax.js`의 청크 후처리가 이미 대응 중인 것과 같은 근본 원인
(`VHS_LoadVideo`류가 요청 프레임 전부를 float32로 미리 만들어둠). 응답 없는 프로세스는
`Stop-Process -Force`로 강제 종료(사용자가 직접 지시) 후 `Start ComfyUI SageAttention.bat`로
재기동해 복구.

**수정** (`openFacePickModal`, `one_node_minimax_h3.js`): 스캔 호출 전에 `getVideoInfo`로 총
프레임수/해상도를 먼저 확인하고, `ui_gallery_minimax.js`의 청크 후처리와 **같은 바이트 예산
공식**(`1.25GB / (w×h×16)`, 8~240 프레임 사이로 clamp)으로 안전 프레임 상한을 계산해서
`frame_load_cap`으로 넘긴다. 클립이 그 상한보다 길면 앞부분만 스캔하고 모달에 경고를 띄운다
(그 지점 이후 컷은 안 잡힘). **실제 렌더(`buildFaceRefineGraph`)의 `frame_load_cap`은 그대로
0(전체) — 이건 미리보기용 스캔에만 거는 안전장치지, 최종 결과물을 자르는 게 아니다.** 아주 긴
클립을 실제로 리파인하는 것 자체의 메모리 예산은 아직 안 다룸(§14 열린 질문).

---

## 14. 열린 질문 — 아주 긴/큰 클립의 실제 렌더 메모리 예산

§13의 수정은 **Pick Faces 미리보기 스캔**만 안전하게 만든 것이고, `runFaceRefine()`이 실제로
큐에 올리는 렌더 그래프(`VHS_LoadVideo`/`H3FaceSelect`의 `frame_load_cap: 0`)는 여전히 클립
전체를 한 번에 메모리에 올린다 — LTX Upscale이 이미 겪었던 것과 같은 종류의 문제
(`ltxSegmentMode`/세그먼트 분할로 대응한 그것)인데, Face Refine은 프레임수가
`H3FaceTrackCrop`의 출력(그래프 실행 시점에만 알 수 있음)이라 LTX Upscale처럼 JS에서
미리 세그먼트 계획을 짤 수가 없다 — §4/§10에서 "세그먼트 없음"이라고 못박은 이유가 이거다.

**해결 방향(구현 안 함, 다음 라운드 후보)**: `getVideoInfo`로 소스 길이를 미리 재서, 안전
상한(§13과 같은 바이트 예산)을 넘는 클립이면 렌더 전에 경고하거나, `skip_first_frames`/
`frame_load_cap`으로 소스 자체를 윈도우 단위로 나눠 **여러 번 큐잉 + 결과를 이어붙이는** 방식
...
**추가 검증 결과 (재테스트, 같은 날)**: 0.45로 올려도, 그리고 identity_track을 아예 꺼도
**증상이 완전히 동일**했다 — 즉 identity는 이 버그의 원인이 아니었다(가설 기각). 대신
Confidence를 0.35 → 낮춤으로 테스트하니 드리프트가 더 오래 버티다가, 결국 **픽한 인물이
카메라에 등을 보이는 순간(스핀 동작 중 뒷모습)**에 다른(인접하지 않은) 사람으로 넘어갔다.
사용자가 회전하지 않는 클립으로 재테스트해보니 완벽하게 유지됨 — **원인이 "얼굴 트래킹
전반의 약점"이 아니라 "픽한 인물이 한 바퀴 스핀하며 완전히 등을 돌렸다가 다시 정면으로
돌아오는 구간에서만" 발생하는 것으로 좁혀짐. 자세한 코드 리딩(`_track_continuity`,
conflict resolution, `_continuity_cost`)까지 마쳤지만 이건 서드파티 팩(`nodes.py`)
자체의 재획득(re-acquire) 로직 문제라 우리 쪽 설정으로는 근본 해결이 안 됨 — 로컬 패치는
업데이트 시 유실되므로, 사용자 판단으로 로컬 패치 대신 업스트림에 이슈 등록:
[Carasibana/ComfyUI-H3-FaceRefine#14](https://github.com/Carasibana/ComfyUI-H3-FaceRefine/issues/14)
(+ 스핀/뒷모습 조건을 특정한 후속 코멘트). 당장은 워크어라운드 없이 이 한계를 인지한 채
사용 — 업스트림 수정을 기다리는 상태.**

**해결됨 (2026-09-13)**: 개발자(Carasibana)가 확인 후 **v1.1.2**에서 수정 — 얼굴이 사라졌을 때
(고개 돌림/스핀/가림) 다른 가장 가까운 얼굴로 넘어가던 걸, 이제는 "원래 위치에서 도달
가능한 범위 안의 얼굴로만" 이어가도록 변경. 이어갈 수 없는 프레임은 interpolate + 원본
픽셀 유지, 다시 나타나면 재획득. identity_track on/off 관계없이 동일하게 동작. 리포트에
`lost:` 줄이 새로 추가돼 그런 프레임들을 나열하고 프리뷰에서 빨간색으로 표시. `git pull`로
d7ae3ee(1.1.1) → d8521d1(1.1.2) 업데이트 완료, 우리가 쓰는 파라미터명
(`identity_threshold`/`identity_track`/`fallback_detector`/`confirmed_pick`) 전부 그대로라
호환성 문제 없음. 사용자가 동일한 3인 스핀 클립으로 재테스트 — **드리프트 사라짐, 정상
동작 확인**. 이슈 #14에 확인 코멘트 남기고 종료.
