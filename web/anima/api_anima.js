// api_anima.js — Backend communication for Anima ONE STUDIO (TJ)
import { api } from "../../../scripts/api.js";
import { API, SUBFOLDER } from "./core_anima.js";
...
export async function getGallery({ offset = 0, limit = 48, subfolder = SUBFOLDER, favonly = false } = {}) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const onExecuted = (ev) => {
      try {
        ...
        if (!ev.detail?.output?.images?.length) return;
        cleanup(); resolve(ev.detail);
      } catch(e) { cleanup(); reject(e); }
    };
    const onExecError = (ev) => {
      if (ev.detail?.prompt_id && ev.detail.prompt_id !== promptId) return;
      cleanup(); reject(new Error(ev.detail?.exception_message || "generation failed"));
    };
...
export async function saveMeta(filename, subfolder, stateObj) {
  ...
  } catch(e) { console.warn("[Anima] saveMeta:", e); }
}
