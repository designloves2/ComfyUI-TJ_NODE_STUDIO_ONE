...
export function createGalleryOverlay(state, ctx, onReuse, onSendTo) {
  ...
  topRow.appendChild(el("div", { text: "Gallery — Anima ONE STUDIO", style: { color: "#fff", fontSize: "14px", fontWeight: "700", flex: "1" } }));
  ...
  // NOTE: no overflow:hidden on this grid — CSS Grid auto-min-size can collapse a
  // grid item with overflow:hidden to zero. Cells stay auto-sized instead.
      ...
      const b = btn(t.label, async () => {
        ...
        try {
          if (t.field) {
            ...
            // T2I has no image input — "send" there means reuse this image's saved settings.
            const meta = await loadMeta(img.filename, img.subfolder || "");
            closeViewer(); ov.style.display = "none";
            if (meta && typeof onReuse === "function") onReuse({ ...meta, mode: "t2i" });
          }
