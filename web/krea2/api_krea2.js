// api_krea2.js — Backend communication for Krea 2 ONE STUDIO (TJ)
import { api } from "../../../scripts/api.js";
import { API, SUBFOLDER } from "./core_krea2.js";
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const onExecuted = (ev) => {
      try {
        ...
        if (!ev.detail?.output?.images?.length) return; // wait for image node
...
export async function saveMeta(filename, subfolder, stateObj) {
  try {
    const meta = Object.fromEntries(
      Object.entries(stateObj).filter(([k]) => !["upscaleImageData"].includes(k))
    );
  ...
  } catch(e) { console.warn("[K2] saveMeta:", e); }
}
