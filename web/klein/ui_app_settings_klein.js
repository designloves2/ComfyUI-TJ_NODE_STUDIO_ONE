// ui_app_settings_klein.js — Settings overlay for flux2 klein One (TJ)
...
export function createSettingsOverlay(state, ctx) {
  ...
  topRow.appendChild(el("div", { text: "⚙ Settings — flux2 klein One (TJ)", style: { color: "#ffffff", fontSize: "14px", fontWeight: "700", flex: "1" } }));
  ...
  getConfig().then(cfg => {
    ...
    // Was only ever set as a placeholder (grey hint text) — state.saveSubfolder stayed
    // empty, so every render/gallery/graph fell back to the hardcoded default subfolder
    // instead of the path actually saved here.
