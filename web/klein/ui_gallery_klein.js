...
export function createGalleryOverlay(state, ctx, onReuse, onSendTo) {
  ...
  function openViewer(img, imgIdx) {
    ...
    // 눈가리기 — the fullscreen viewer used to load the real file unconditionally, so a
    // hidden image (and every one you'd land on with ← / →) showed in full here even
    // though its grid thumbnail was blurred. It never gets a real <img src> while hidden;
    // the shade's own 👁 (or the thumbnail's) is what reveals it.
